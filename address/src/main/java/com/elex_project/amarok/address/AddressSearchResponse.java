...
public final class AddressSearchResponse
		extends BaseResponse<AddressSearchResult> {

	AddressSearchResponse(final HttpResponse<String> response) throws JsonProcessingException {
		log.debug("Rx({}): {}", response.statusCode(), response.body());
		setHttpStatus(response.statusCode());
		setBody(objectMapper
				.readValue(response.body(), AddressSearchResult.class));
	}
}
