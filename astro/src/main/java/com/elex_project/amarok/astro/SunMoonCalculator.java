...
public class SunMoonCalculator {
	...
	private static final double TWO_PI_INVERSE = 1.0 / TWO_PI;
	 ...
	 * @param m Minute.
	 * @param s Second.
	 * @param obsLon Longitude for the observer. "DEG"
	 * @param obsLat Latitude for the observer. "DEG"
	...
	public SunMoonCalculator(int year, int month, int day, int h, int m, int s,
		...
		obsLon = Math.toRadians(obsLon); obsLat = Math.toRadians(obsLat);
	...
	public static String getDateAsString(double jd, ZoneId localZoneId, @Nullable DateTimeFormatter formatter) throws Exception {
		if (jd == -1) return "NO RISE/SET/TRANSIT FOR THIS OBSERVER/DATE";
		if (null==formatter) formatter = DateTimeFormatter.ISO_DATE_TIME;

		int date[] = SunMoonCalculator.getDate(jd);

		ZonedDateTime zonedDateTime = ZonedDateTime.of(
				LocalDateTime.of(date[0],date[1],date[2],date[3],date[4],date[5]), ZoneId.of("UT"));

		LocalDateTime dateTime = zonedDateTime.withZoneSameInstant(localZoneId).toLocalDateTime();
